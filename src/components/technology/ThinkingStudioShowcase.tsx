      ...
      <div className="container">
        ...
        <div className="deliberation-studio-card spotlight-card" id="thinking-tree-container">
          ...
          <div className="deliberation-topbar">
            ...
            <div className="deliberation-actions" role="tablist" aria-label="Deliberation stages">
              {branches.map((b) => (
                <button
                  key={b.key}
                  type="button"
                  className={`delib-branch-btn ${b.key === 'assumptions' ? 'active' : ''}`}
                  data-delib-tab={b.key}
                >
          ...
          {/* Focused Deliberation Workspace */}
          <div className="deliberation-workspace-grid">
            {/* Left Column: Question Anchor */}
            <div className="deliberation-anchor-card">
              ...
              <div className="inquiry-meta-row">
                ...
                <span className="inquiry-stage-count" id="delib-stage-count">Stage 01 of 05</span>
              </div>
            </div>

            {/* Right Column: 5 Stage Panels (only active is visible) */}
            <div className="deliberation-stage-panels">
              {branches.map((b) => (
                <div
                  key={b.key}
                  className={`deliberation-stage-card ${b.key === 'assumptions' ? 'active' : ''}`}
                  id={`delib-panel-${b.key}`}
                  data-delib-panel={b.key}
                  style={{ display: b.key === 'assumptions' ? 'block' : 'none' }}
                >
                  <div className="stage-card-header">
                    <div className="stage-card-badge">
                      <span className="stage-badge-num">{b.num}</span>
                      <span className="stage-badge-label">Stage: {b.stageLabel}</span>
                    </div>
                  </div>

                  <div className="stage-block stage-inquiry-block">
                    <span className="stage-subhead">Inquiry Focus</span>
                    <p className="stage-inquiry-text">{b.inquiry}</p>
                  </div>

                  <div className="stage-block stage-examination-block">
                    <span className="stage-subhead">Critical Examination</span>
                    <p className="stage-examination-text">{b.examination}</p>
                  </div>

                  <div className="stage-finding-strip">
                    <span className="stage-finding-label">Examination Note</span>
                    <p className="stage-finding-text">{b.finding}</p>
                  </div>
                </div>
              ))}
            </div>
